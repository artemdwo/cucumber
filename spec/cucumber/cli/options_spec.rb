      ...
      describe 'parsing' do
        ...
        context 'handling multiple formatters' do
          ...
          it "catches multiple profile formatters using the same stream" do
            given_cucumber_yml_defined_as({'default' => '-f pretty -f progress'})
          ...
          it "replaces the STDOUT profile formatter with the STDOUT command line formatter" do
            given_cucumber_yml_defined_as({'default' => '-f progress'})
            options = Options.new(output_stream, error_stream, :default_profile => 'default')
          ...
          it "catches multiple profile and command line formatters using the same stream" do
            given_cucumber_yml_defined_as({'default' => '-f junit -o result.xml'})
            options = Options.new(output_stream, error_stream, :default_profile => 'default')

            expect{ options.parse!(%w{-f json -o result.xml}) }.to raise_error("All but one formatter must use --out, only one can print to each stream (or STDOUT)")
          end

          it "allow multiple identical formatter definitions" do
            given_cucumber_yml_defined_as({'default' => '-f junit -o result.xml'})
            options = Options.new(output_stream, error_stream, :default_profile => 'default')

            expect{ options.parse!(%w{-f junit -o result.xml}) }.not_to raise_error
          end
        end
